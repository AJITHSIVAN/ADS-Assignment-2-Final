...
for value in df_no2.columns[2:].values:
    ...
    df = pd.DataFrame(data=df, columns=[
                      "NO2", "Cereal Yield", 'Arable land (% of land area)', 'Agricultural land (% of land area)'])
    df['Cereal Yield'] = df_yield[value].values
    df['NO2'] = df_no2[value].values
    df['Arable land (% of land area)'] = df_no2[value].values
    df['Agricultural land (% of land area)'] = df_no2[value].values
...
plt.bar(x_axis - 0.05, df_no2_new['Bangladesh'],
        width=0.05, label='Bangladesh')
